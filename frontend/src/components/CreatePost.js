...
const CreatePost = ({ currentUser, refetchPosts }) => {
  ...
  const handlePrice = event => {
    setPost({ price: event.target.value });
  };
  ...
  const submitPost = async () => {
    ...
    if (!post.price) {
      toast("price is mandatory", { type: "error" });
      return;
    }
  ...
  return (
    <>
      <div
        className="editor mx-auto w-10/12 flex flex-col text-gray-800 border rounded-lg border-gray-100 p-4 shadow-lg max-w-2xl">
        <textarea
